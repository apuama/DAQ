...
class MLX90614():
    ...
    def __init__(self, address=0x7, bus_num=1):
...
print ('Temperatura do objeto de teste (07): ')
...
f = open ("/home/pi/Desktop/tempir07.txt",  'a')#abre o arquivo na froma de leitura e na froma "append" (significa que sempre salva no fim do arquivo referente sem sobreescreve-lo)
f.write (str(hoje))
f.write('\nInfravermelho 07')
...
while(1):
    try:
        if __name__ == "__main__":
            ...
            f = open ("/home/pi/Desktop/tempir07.txt",  'a')
            f.write('\n')
            f.write(str(datetime.now().time()))
            f.write(': ')
            f.write(str(valor))
            f.close
            time.sleep(0.4)		#delay de 0.5 segundo
